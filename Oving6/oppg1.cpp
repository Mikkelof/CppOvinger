  ...
  class Knight : public Piece {
    ...
    bool valid_move(int from_x, int from_y, int to_x, int to_y) const override {
      if (((abs(to_x - from_x) == 1) && (abs(to_y - from_y) == 2)) ||
          ((abs(to_x - from_x) == 2) && (abs(to_y - from_y) == 1))) {
        return true;
      } else {
        return false;
      }
...
public:
  ...
  ChessBoardPrint(ChessBoard &board) : board(board) {
    ...
    board.after_piece_move = [](const ChessBoard::Piece &piece, const string &to) {
      for (size_t i = board.squares.size() - 1; i + 1 > 0; i--) {
        cout << "  " << i + 1 << "  ";
        for (size_t j = 0; j < board.squares.at(i).size(); j++) {
          if (!board.squares.at(j).at(i)) {
            cout << "      x      ";
          } else {
            auto &piece = board.squares.at(j).at(i);
            cout << " " << piece->print() << " ";
          }
        }
        cout << endl;
